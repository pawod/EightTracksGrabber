...
namespace Com.Wodzu.EightTracksGrabber.Messages
{
    /// <summary>
    ...
    public class SongResponse : ISongResponse
    {
            ...
            // TODO: make configurable
        {
            Task.Factory.StartNew(() =>
                                  {
                                      // TODO: download playlist pic to playlist dir
                                      var target = GetDownloadTarget(targetBaseDir);
                                      if (!overwrite && FileNameExists(target))
                                          // if audiofile already exists, don't create a .download file
                                      {
                                          Logger.Info(
                                              "An audio file with name \"{0}\" already exists, aborting download",
                                              FileName);
                                          return;
                                      }
                          ...
                          .Any(
                              file =>
                                  Path.GetFileNameWithoutExtension(file.FullName).
                                       Equals(FileName, StringComparison.InvariantCulture)));
        }

        private void FinishDownload(FileInfo target, bool generateTags)
        {
            var container = AudioContainer.GetContainerFormat(target.FullName);
        ...
        private FileInfo GetDownloadTarget(string targetBaseDir)
        {
            ...
            return
                new FileInfo(String.Format("{0}{1}{2}{3}{4}{5}{6}.download",
                    targetBaseDir,
